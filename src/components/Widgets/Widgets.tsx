...
import { selectCurrentToken, selectUserData } from "../../features/api/auth/authSlice";
...
const Widgets = () => {
    ...
    const filteredList = (token && widgets)
        ? widgets.map(widget => widgetList.find(item => item.id === widget)).filter(widgetDoesExist)
