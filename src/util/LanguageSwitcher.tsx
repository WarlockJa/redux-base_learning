...
const LanguageSwitcher = ({
  ...
  const lngOptions = Object.entries(languages).map((entry, index) => (
    <option key={index} value={entry[1].nativeName}>
      {fullDescription ? entry[1].nativeName : entry[0]}
    </option>
  ));

  // switching languages on item select
  const switchLanguage = async (event: ChangeEvent<HTMLSelectElement>) => {
    const selectedValue = Object.keys(languages).find(
      (lng) => languages[lng].nativeName === event.target.value
    );
    i18n.changeLanguage(selectedValue);
    // changing idtoken locale in store
    dispatch(
      setIdToken({
        idToken: { ...idToken, locale: selectedValue ? selectedValue : "en" },
      })
    );
